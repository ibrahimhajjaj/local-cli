...
ACTIONS:
    ...
    wp           Runs WP-CLI commands
    <command>    Runs any shell command directly in the site container
...
EXAMPLES:
    ...
    local-cli sg ls -la
    local-cli sg git status

    # Run WP-CLI commands directly
    local-cli sg wp option get home
    local-cli sg wp plugin list
`)
}
...
func runAction(scriptPath string, site Site, action string, extraArgs []string) {
	...
	default:
		// Treat the action as the command itself, and append any extra args.
		// Example: local-cli site ls -la -> command = "ls -la"
		fullCmd := []string{action}
		if len(extraArgs) > 0 {
